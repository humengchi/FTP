...
#define AndV(val1, val2, message) { And(val1, val2); if (!val2) DDLogInfo(message); }
...
- (BOOL) _checkFMServer:(FMServer*)server {
    BOOL success = YES;
    AndV(success, (server != nil), @"ftp上传：FMServer check failed: server cannot be nil");
    Check(success);
    AndV(success, (server.destination != nil), @"ftp上传：FMServer check failed: destination cannot be nil");
    AndV(success, (server.username != nil), @"ftp上传：FMServer check failed: username cannot be nil");
    AndV(success, (server.password != nil), @"ftp上传：FMServer check failed: password cannot be nil");
    AndV(success, (server.port > 0), @"ftp上传：FMServer check failed: port cannot be negative");
    return success;
}
...
- (BOOL) _uploadFile:(NSURL*)fileURL toServer:(FMServer*)server {
    ...
    AndV(success, (finalURL != nil), @"ftp上传：finalURL为空！");
    Check(success);
    
    self.fileReader = [[NSInputStream alloc] initWithFileAtPath:fileURL.path];
    AndV(success, (self.fileReader != nil), @"ftp上传：fileReader为空！");
    Check(success)
    [self.fileReader open];
    
    CFWriteStreamRef writeStream = CFWriteStreamCreateWithFTPURL(NULL, (__bridge CFURLRef)finalURL);
    AndV(success, (writeStream != NULL), @"ftp上传：writeStream为空！");
    Check(success)
    self.serverStream = (__bridge_transfer NSOutputStream*) writeStream;
    
    AndV(success, [self.serverStream setProperty:server.username forKey:(id)kCFStreamPropertyFTPUserName], @"ftp上传：serverStream设置用户名错误！");
    AndV(success, [self.serverStream setProperty:server.password forKey:(id)kCFStreamPropertyFTPPassword], @"ftp上传：serverStream设置密码错误！");
    Check(success)
...
- (BOOL) uploadFile:(NSURL*)fileURL toServer:(FMServer*)server {
    if (![self _checkFMServer:server]) {
        DDLogInfo(@"ftp上传：checkFMServer failed！");
        return NO;
    }
    if (!fileURL) {
        DDLogInfo(@"ftp上传：fileURL为空！");
        return NO;
    }
    BOOL isDir;
    if (![[NSFileManager defaultManager] fileExistsAtPath:fileURL.path isDirectory:&isDir] || isDir) {
        DDLogInfo(@"ftp上传：fileURL路径不存在或为目录！");
        return NO;
    }
